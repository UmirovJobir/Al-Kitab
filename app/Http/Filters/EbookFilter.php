...
class EbookFilter extends Filter
{
    /**
    ...
    {
        return $this->builder->whereHas('ebook', function ($query) use ($value) {
    ...
    {
        return $this->builder->whereHas('ebook', function ($query) {
...
//                ->orderBy('sold_quantity', 'asc');
            ...
            if (!Schema::hasColumn('ebooks', $value)) {
                return $this->builder->orderBy($value, 'desc');
            } else {
                return $this->builder->leftJoin('ebooks', 'books.id', '=', 'ebooks.id')
                    ->orderBy('ebooks.' . $value, 'desc')
                    ->select('books.*');
            }
        }
    }
