...
class AbookFilter extends Filter
{
    /**
    ...
    {
        return $this->builder->whereHas('abook', function ($query) use ($value) {
    ...
    {
        return $this->builder->whereHas('abook', function ($query) {
...
//                ->orderBy('sold_quantity', 'asc');
            ...
            if (!Schema::hasColumn('ebooks', $value)) {
                ...
                return $this->builder->leftJoin('abooks', 'books.id', '=', 'abooks.id')
                    ->orderBy('abooks.' . $value, 'desc')
                    ->select('books.*');
            }
        }
    }
